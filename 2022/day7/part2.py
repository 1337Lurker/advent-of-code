...
def main():
    ...
    available_disk_space = DISK_SIZE - root_directory.size()
    space_to_reclaim = MIN_DISK_SPACE - available_disk_space
    directories = [
        directory
        for directory in root_directory.all_directories()
        if directory.size() >= space_to_reclaim
    ]
    directories.sort(key=Directory.size)
